...
An always-on **containerized dev environment** on a remote host, attached over SSH. Inside, AI
coding agents (Claude Code, Codex, pi-coding-agent, opencode) and nvim run under tmux — many in
parallel, each holding working copies of git repositories.
...
2. **Editor-agnostic, not VSCode-locked.** Devcontainers rejected; no `.devcontainer/`. SSH + tmux
   is the canonical attach path.
...
- **Layout (specs/011) — [`docs/layout.md`](docs/layout.md) is the one map.** **project root** ·
  **project config** `.agent-container/` · **user configuration** `~/.config/agent-container/` ·
  **derived host state** · **image sources** `image/`. Never "project directory". Config is two
  levels, project winning, same filename both; **plaintext credentials are user-level only**. Context
  **is** `image/`. **Pre-011 layouts are refused, not ignored.**
...
- **The supported-agent list is single-sourced** (`AGENTS`); a sibling test pins the completions' to
  the CLI's. Both fail on drift and name what to update.
...
- **The inventory remembers what we CREATED; `panic` acts on it.** Durable but **flat**; capped by
  ...
  stops by **compose project label**, and verifies by **observation** — two queries, since
  `already-stopped` needs a pre-snapshot. **Unreachable ⇒ `undetermined`, never `stopped`/`missing`**,
  one fails the run, and an **unverified destroy writes no outcome**.
- **A run's account outlives its container.** The container writes the record to the runs volume
  ...
  **teardown drains before removing volumes**; `task` is recorded verbatim, *unknown* usage never `0`.
- **`doctor` is read-only BY COMPOSITION, not by a flag** — it must never reach `migrate_flat_state`
  or any writer (a test walks the call graph); `unknown` is a first-class status and **never exits 1**;
  a credential is checked by DECLARATION, never resolved (resolving is the prompt).
...
`doctor.md` (checks, statuses, exit codes · 013) ·
`threat-model.md` (**reconcile every feature** — Constitution) · specs/007 (wizard). All under
`docs/`.

## Architecture — keep these layers separate

**image** (`image/`: OS, tmux, sshd, nvim, git, agent CLIs) · **orchestration** (`bin/agent-container`
+ compose) · **entrypoint** (git identity, credential injection, key generation, sshd, tmux) ·
**attach** (`ssh … -t tmux attach`, thin and client-side). Never bake host-specific orchestration
into the image.
...
- **Quality gate — one script, two uses.** `scripts/quality-gate.sh` (roster is the script's); the
  Stop hook and CI run the *same* script. It **excludes** the CI-authoritative acceptance tier
  (`pytest -m acceptance bin/tests`; on macOS+Lima the work dir must be Lima-shared). **Read its exit
  code unpiped** — `| tail` reports tail's status. **Never edit the tree while that tier runs**: it
  re-reads the CLI per invocation, so a mid-edit run measures nothing.
