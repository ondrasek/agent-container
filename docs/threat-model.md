...
| 015 kill switch | ⬜ | **Turns Feature 014's record into an ACTION.** Nothing new is persisted and no credential is read, written or transported — but the same CLI that could previously stop one environment can now stop, or destroy, EVERYTHING across every host in one invocation. **Newly introduced:** an attacker with CLI access on the operator's machine gains a single-command denial-of-service against every environment, and with --destroy a single-command data-destruction path bounded only by the confirmation prompt; the inventory that 014 added as a reconnaissance aid is now also a TARGET LIST. Expected mitigations: destroy requires explicit confirmation, stop is recoverable by design (Constitution I — durable work is pushed, not held), and the action never touches a container absent from the inventory. **Left open:** the confirmation is the only barrier on --destroy, and anyone who can run the CLI can also pass -y. |
| 014 host inventory | ⬜ | |
...
| 019 container-generated push key | ⬜ | **Removes the LAST private key the tool writes to the operator's disk** — the 0644 `<name>.push_key` that `--purge` does not delete — and NARROWS a grant: today the container typically receives the operator's personal push key, i.e. everything that key authorises, where a per-container deploy key authorises one repository. Expected: §3 loses an asset; T1 loses its last write site. **Watch for:** FR-003 puts a private key on the persisted `ssh` volume, amending 003's "outbound key material never lands on a volume" — so a volume that outlives a container now holds signing material, and `--purge` becomes the revocation boundary. Also a **new operator-facing failure mode**: an unregistered key means pushes fail, and this project's first constraint is that agents commit AND push. |
