...
| 018 attach host verification | ⬜ | **REMOVES an exposure rather than adding one**: no private SSH host key is generated, staged or stored on the operator's machine, so the 0644 staged key that survived `--purge` stops existing. Expected: T1 restated (one fewer place a private key can be written); §3 loses an asset. **Watch for a new one** — a tool-managed `known_hosts` becomes the thing that decides whether an attach is trusted, so tampering with it downgrades verification silently, and FR-007's re-pin path is the one place the tool deliberately accepts a changed identity |
