...
| 023 telemetry stack | ✅ | **Reconciled, and the exposure expectation held while three OTHER things went wrong.** §3 gains an asset: a store holding the VERBATIM TASK TEXT of every environment pointed at it, behind a UI with no login. **T15 sharpened, as expected:** 016 accepted verbatim task text for a `0600` file on the operator's own disk; a stack moves the same content into a service. Answered by exposure — `host` by default (reachable from the host and its containers, not from other machines), `network` refused without `-y` on a non-TTY and stating first that it publishes an unauthenticated UI showing task text and an ingest accepting from anyone. **The three questions the expectation raised, answered.** (1) *What does the default actually bind?* More than its name suggests, and the tool now SAYS so: `host` resolves to loopback+bridge under docker and to EVERY INTERFACE under rootless podman, because there is no stable per-host bridge address there — FR-018b makes the resolved addresses printed rather than implied, and a test pins that asymmetry precisely because it is surprising. (2) *Is the task-text consequence stated at widening time?* Yes, in the `network` warning, before anything is created. (3) *Is the unauthenticated ingest as a WRITE surface mitigated by anything other than exposure?* **No, and that is recorded rather than papered over** — anything that can reach the ingest can inject records the operator reads as their own trail, which is T16 escaping the container boundary. Exposure is the only control; the default keeps it off the network. **Newly introduced:** a third-party image the tool pulls and runs (T9's shape applied to infrastructure, Constitution VI intact because nothing links into the tool), and a container that PUBLISHES PORTS — the first this tool creates. **Left open, deliberately:** the UI ships unauthenticated and the tool does not attempt to add authentication to a third-party UI; and retention that cannot be read back is reported as `unconfirmed` rather than failing the deploy, so a store MAY grow unbounded on an image whose settings the tool cannot verify — said aloud rather than assumed away. **T1 unchanged:** the kind is given no credentials and has no credential-delivery path; a test asserts that absence, because a negative security property is the kind that quietly stops being true. |
| 024 interpreting control plane | ✅ | **Reconciled, and the expectation held on every point that mattered.**  **T1**, **T15**, **T16**, **T19**, and a NEW THREAT for the instruction path. This feature makes an agent READ WHAT OTHER AGENTS PRINT AND SPEAK TO THE OPERATOR ABOUT IT, which is a shape the model has not held before. **T16 escapes the container, and that is the headline.** 016 recorded that a run record is the container's own account of itself and not evidence against a misreporting agent; an interpreter *acts on* that account plus the agent's raw output, and its conclusions arrive on the operator's phone carrying the interpreter's credibility. So agent-authored text becomes an input to something the operator trusts — prompt injection with a delivery channel. The reconciliation must show that FR-014 (observed content, never instruction) and FR-011b (record is authority for what happened, log only for what the agent SAID) are enforced by tests and not by the agent's good judgement, since the agent's judgement is exactly what an injection targets. **The mitigation that makes the rest tolerable is an ABSENCE**: FR-020 gives the interpreter no credential that can change the fleet, and the absence is structural — `image/Dockerfile` installs no container runtime client, so `detect_runtime()` cannot resolve and every management command refuses, which is 017's control-plane-image reasoning inverted deliberately. FR-020a additionally forbids leaving a dormant action path against the future authority feature. Both need the call-graph reachability guard (013's `doctor` technique), and the guard needs an entry in `test_guards_can_fail.py`. **T15 crosses a TRUST BOUNDARY it has not crossed before, and this is the part to read twice.** 023 moved verbatim task text into a service with no login but kept it on the operator's own host behind `host` exposure; 024 sends task text AND the agent's full output into a **Slack workspace** — readable by everyone in the bound conversation, by workspace administrators, and under the workspace's retention policy. That is the operator's content leaving their trust domain to a third party, and no exposure level bounds it because the boundary is a company's SaaS account. FR-024a makes it a stated consequence before creation rather than a discovery after; the reconciliation owes an answer on whether stating it is enough, given 020's finding that a stated consequence is visible only to an operator who reads it. **Newly introduced, expected:** (1) a SECOND INBOUND INSTRUCTION PATH that opens no port — the container polls Slack outbound, so nothing is published and nothing is firewallable, but instructions still arrive, and the admit set for that path is a declared workspace member (FR-023) rather than sshd's `authorized_keys`; (2) a credential whose blast radius is *speak as the interpreter to the operator*, which is a class this tool has not held before — it grants no access, it grants VOICE, and a holder can tell the operator the fleet is fine; (3) the agent's output stream as a durable exported artifact, materially wider than 016's single operator-authored `task` field, governed by its own named exclusion (`export_agent_logs`) because one switch over both would let an operator who excluded the task believe they had excluded the wider thing. **Expected to be LEFT OPEN:** forged interpretations — the stack's ingest is unauthenticated by design (023), so anything that can reach it can write a signal the operator reads as an assessment, and exposure remains the only control; the mitigation is that every interpretation names the interpreter claiming it, which makes an impostor distinguishable rather than prevented. Authenticating the ingest is a change to 023, not to this feature. **T19 widens:** log export is a second continuous outbound channel from every container, still fail-open and still governed by a 012 declaration. **WHAT WAS BUILT, AGAINST WHAT WAS EXPECTED.** (1) *T16 escaping the container* — confirmed as the headline and closed structurally rather than behaviourally: the agent image installs NO container runtime client and an interpreter is never given the control plane's standing key, so `detect_runtime()` cannot resolve and every management command refuses. Both halves are asserted by tests, one walking the transitive closure of `__code__.co_names` from every `interpret` command (013's `doctor` technique) and one reading the Dockerfile's install lines. The guard is itself shown to FAIL against `do_up`, because a guard nobody has seen fire is decoration. FR-014 and FR-011b are enforced by tests, not by the agent's judgement — which is what an injection targets: supervisor-addressed text is quoted as a finding and changes nothing, and a claim contradicting the record is reported AS a disagreement with the record as authority. The directive detector is deliberately broad and deliberately not a control; a missed match costs a flag, never an action, because nothing in the bridge can act (asserted: no subprocess, os.system, exec or eval in the file). (2) *T15 crossing a trust boundary* — confirmed, and it is the feature's real cost. Task text AND full agent output now reach a Slack workspace readable by everyone in the conversation, by workspace administrators, and under their retention policy. No exposure level bounds it because the boundary is a third party's SaaS account. FR-024a states it before creation, in full, and the 020 finding applies unchanged: a stated consequence is visible only to an operator who reads it. (3) *The inbound path* — opens NO port; the container polls Slack outbound. The admit set is a declared workspace member with no default that admits anyone, and an undeclared sender gets SILENCE plus a recorded refusal, because replying confirms to a stranger that something is listening. **NEWLY INTRODUCED, AS EXPECTED:** a credential whose blast radius is VOICE rather than access — whoever holds the bot token can tell the operator the fleet is fine — delivered over the container's own sshd like every other secret (asserted: classified secret, routed to `deliver_secrets`, absent from the deployment description). **NEWLY INTRODUCED, NOT ANTICIPATED:** the agent OUTPUT STREAM as a durable exported artifact, materially wider than 016's single operator-authored `task` field — and the interaction that came with it, found by the acceptance tier rather than by review: **`export_task_text: false` stopped making tasks private**, because agents echo their task and the log stream re-exported it. An undeclared log switch now inherits the operator's stated caution; an explicit one is obeyed in both directions and warned about. **LEFT OPEN, DELIBERATELY:** forged interpretations — 023's ingest is unauthenticated by design, so anything that can reach it can write a signal the operator reads as an ASSESSMENT, which is worth more to an attacker than a forged record; exposure is the only control and every interpretation names the interpreter claiming it, which makes an impostor distinguishable rather than prevented. Also left open: a dead `tee` kills the agent with SIGPIPE (measured, exit 141), so log export can fail a working run — documented at the tee with the fix that was not taken and what it would cost. And `watched_scope` is validated as a NAME, which removes shell metacharacters and paths but cannot separate a hostname from a secret that happens to look like one; 014's no-free-text property is accurate about the SHAPE of what is stored, not a guarantee about its meaning. **T19 widened:** log export is a second continuous outbound channel from every container, still fail-open, still governed by a 012 declaration. |
| 025 podman secrets | ⬜ | expected: **T1**, and unusually for this project the expectation is that the row will record an exposure REMOVED rather than added. Every credential the tool delivers today passes through the tool: Constitution IX makes the push over the container's own sshd the one legitimate channel, and it is the right answer to the question it was asked, but it answers by making the tool a COURIER — and a courier can drop what it carries, into a traceback, a `--json` field somebody adds later, a debug log, a process table. 019 removed a whole class of this by having the container generate its own SSH key, which §3 records as *eliminated by design* rather than mitigated. A REFERENCED podman secret is that same move for credentials a container cannot generate itself: the operator creates it, the runtime mounts it, the tool knows only a name. **The reconciliation must confirm the boundary held, because the whole benefit is a negative:** FR-002 forbids the tool reading the value for ANY purpose including validation and error messages (a single read that 'just checks' restores exactly what this removes), and FR-003/FR-003a forbid creating one — creating means sending plaintext over the runtime channel, which is what IX forbids and why, measured: a `tcp://` context carries it in the clear. Expect that to be asserted STRUCTURALLY, the way 013 proves `doctor` read-only by composition and 024 proves an interpreter cannot act, rather than observed on whichever paths a test happened to exercise. **Newly introduced, expected:** a credential route the tool cannot revoke. Constitution IX's persistence clause pairs persistence with RECONCILIATION and forbids separating them — whatever holds a secret must be removed when the operator stops declaring it, or the declaration stops being the authority. That clause CANNOT be honoured for a value the tool does not own: `--purge` must not remove an operator's secret, which may serve containers this tool knows nothing about. So the feature trades revocability for non-possession, and the reconciliation owes an explicit answer on whether stating that at `--purge` (FR-011) is sufficient or whether IX needs amending — narrowing the claim quietly is the one outcome that would be worse than either. **Also expected to be left open:** `podman secret` stores values unencrypted by default, so this route changes who holds a value IN TRANSIT and within this tool and does not harden the host (FR-013 forbids implying otherwise); a rotated secret does not reach a running container (FR-012); and a secret belongs to the user running podman, so 'no such secret' and 'not yours' are different facts that must not be reported identically. **Expected to add NO channel:** the value crosses nothing, which is the point — for a remote host it is already there, and the daemon-side nature that broke `configs: {file:}` delivery in 001/003 is what makes this one work. **T17/T18 untouched:** FR-017 forbids this becoming a path for an SSH private key, which 018/019 removed entirely, and FR-018 keeps the tool minting nothing. |
| Constitution IX repair — all credentials pushed (2026-08) | ✅ | **REMOVES a standing violation of a MUST principle, found by running the tool rather than reading it.** A declared credential was routed by NAME: `anthropic`/`openai` (two spellings each) took the compliant pushed channel, and **everything else** — `GH_TOKEN`, every other provider — was written in plaintext to `state/<host>/<name>.cred.env`, which the generated compose model then referenced with `env_file:`. Constitution IX names that case explicitly ("written to a staging file that the description references"), and it is the second clause precisely because it is the repair that looks like an improvement. **Measured:** the value was read back out of that file, and the compose reference was read out of the generated model. §3 loses a plaintext artifact that persisted after every deploy at 0600, and T1 loses a place the tool writes credential material. **Newly introduced:** a second secret KIND (`env/<NAME>`) in the receive-secret allowlist, and the ref charset now admits uppercase so a variable name rides verbatim. Neither widens traversal — the charset still bars `.` entirely, so `..` cannot be formed — and the KIND is still checked against a closed list, so a mis-cased kind is refused by the second guard rather than the first (asserted). Each env credential gets its own volume like any other, so `creds remove` and the deploy-time prune work on it unchanged. **Left open, deliberately:** the operator's OWN env file may still contain a token and is still referenced by the model — that file is the operator's material, written by them, and the tool now passes it through untouched rather than copying it; a stale `<name>.cred.env` from before the fix is DELETED on the next deploy with a warning, since nothing else would ever rewrite it. Also: `plan` now refuses an environment with no env file, which `apply` always did — a precondition checked at the surface that can see it rather than left to fail later. |
