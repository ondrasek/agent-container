...
| 023 telemetry stack | ⬜ | expected: **T15**, **T19**, **T12**, and one genuinely NEW exposure. The tool has never before published a LISTENING SERVICE. Agent containers and control planes are reached over sshd with a declared admit set; this kind ships an UNAUTHENTICATED UI and an UNAUTHENTICATED INGEST, so the entire access-control story collapses into one decision — which address they bind to. That is why the spec makes loopback the default (FR-018), makes widening an explicit act that must SAY what it exposes (FR-019), and forbids publishing as a side effect of any other choice (FR-020); the reconciliation owes evidence that no code path reaches a routable bind without the operator naming it. **T15 sharpened, and this is the row's centre of gravity:** task text is exported BY DEFAULT (017 FR-009f), so a stack accumulates the verbatim tasks — and, where an agent exports them, the prompts and assistant responses — of every environment pointed at it. Feature 016 accepted that risk for a `0600` file on the operator's own disk; a stack moves the same content into a service with no login. An operator who widens exposure to 'just have a look from my laptop' publishes their agents' instructions, and the reconciliation must show that consequence is stated at that moment rather than in documentation. **New: an ingest nobody authenticates is a WRITE surface.** Anything that can reach it can inject records the operator will read as their own trail, which is T16 (a record is an account, not evidence) escaping the container boundary — previously an agent could only lie about its OWN run. Mitigated only by exposure, which is the argument for the default. **T19 restated in reverse:** 017 treated export as an outbound channel the boundary governs; this feature makes the destination something the tool itself created, so a stack on a host an environment can reach must still be DECLARED to be exportable to — the boundary does not stop applying because the endpoint is ours. **T12:** the silent-failure surface is removal. Export fails open, so deleting a stack stops telemetry from every environment still pointed at it without any error anywhere — the spec makes that consequence something removal states (Edge Cases), and the reconciliation should confirm it is said rather than implied. **Adds no credential channel:** the kind holds no secrets, is given none (FR-004), and runs no sshd — the exposure decision is what replaces those questions, not an addition to them. **Newly introduced dependency:** a third-party image the tool pulls and runs; Constitution VI is not breached because nothing links into the tool, but the fleet now includes a container whose contents the project does not build, which is T9's shape applied to infrastructure rather than to agent CLIs. |
