...
#   1. unset DEVENV_TMUX_WINDOWS -> windows 'shell edit agents' exist, in order,
...
#   3. explicit-empty DEVENV_TMUX_WINDOWS='' -> exactly ONE window (opt-out).
...
cat > "${STUB}/tmux" <<'EOF'
#!/usr/bin/env bash
exec "${DEVENV_REAL_TMUX}" -L "${DEVENV_TMUX_SOCKET}" "$@"
EOF
chmod +x "${STUB}/tmux"
...
run_entrypoint() {
    ...
    (
        ...
        export DEVENV_REAL_TMUX="${REAL_TMUX}" DEVENV_TMUX_SOCKET="${SOCK}"
