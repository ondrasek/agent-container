...
- **FR-015**: The generated artifact MUST express the **injected SSH identity** (private host key and public authorized-keys) as declarative injected material sourced from the operator's local machine, so it is transferred to a remote host over the runtime context, rather than as a local-filesystem bind that would resolve empty on a remote host. The private host key MUST be kept off the image and off any command line (Constitution III). **As implemented:** both are delivered as compose **`configs`** (0644), not `secrets` — a compose `secret` with an absolute `target` crash-loops the container on some daemons, and 0600-staged secrets are unreadable by the container's `dev` uid (host uid ≠ container uid). The host key's secrecy is thus enforced by its per-host, non-committed state location rather than by the compose `secret` primitive; see R5 and `contracts/provisioner.md`.
...
- **Credentials**: provider API tokens and SSH keys are injected at runtime from the operator's environment/config, never baked, never on argv (Constitution III). **Correction (US2, as implemented):** provisioning DOES introduce one tool-managed stored secret — a per-host, file-based **automation private key** (state dir, 0600) authorized on the server's root so the tool's docker socket-forward signs unattended (the operator's own key may be interactive/agent-gated). It is host-scoped, never on argv, and destroyed with the host; the earlier "no stored long-lived secret" claim no longer holds literally, though the key remains Least-Exposure-compliant.
