...
- [X] T001 Add the module constant `RESOLVER_TIMEOUT = 30` (seconds) near the other credential constants in `bin/agent-container`, with a comment citing the research R5 rationale (a pre-unlocked manager resolves fast; 30 s covers a network round-trip while guaranteeing a wedged CLI can never hang an apply, FR-005).
...
- [X] T002 [P] Write failing tests in `bin/tests/test_agent_as_code.py` for `_run_resolver`: runs the argv **directly with no shell** (assert no `shell=True`; a metacharacter in an argument is passed through literally, never interpreted); **`stdin` is closed** (non-interactive, FR-005); a **timeout** raises a `die` rather than hanging (FR-005); a **missing binary**, a **non-zero exit**, and an **empty stdout** each `die` naming the credential (FR-004); **a whitespace-only stdout (e.g. just `"\n"`) also `die`s** — it would otherwise survive the empty check and become an **empty secret** once delivery strips the trailing newline (FR-004 hole, analyze C2); the resolver's **stderr is NEVER echoed** into the message (FR-006, Constitution III) — assert a secret planted on the resolver's stderr does not appear in the raised message; and the failure message **carries a non-specific remediation hint** (e.g. the manager may need unlocking) so suppressing the resolver's own diagnostic still leaves the failure actionable (FR-006).
- [X] T003 Add `_run_resolver(argv, name, *, timeout=RESOLVER_TIMEOUT) -> str` in `bin/agent-container` — `subprocess.run(argv, capture_output=True, text=True, stdin=subprocess.DEVNULL, timeout=…)`, no shell; on `TimeoutExpired`/`OSError`/non-zero `die` with a **generic, secret-free** message naming the credential (never include the resolver's stderr); **test the emptiness of the STRIPPED output** (`if not out.strip(): die(...)`) so a whitespace-only result cannot become an empty secret after delivery's `rstrip` (analyze C2); return stdout unmodified (delivery owns newline normalization). Generalized from the existing `_run_decrypt`, which **remains in place until T011 removes the `encrypted` source** (both coexist if US3 is deferred — analyze L1).
- [X] T004 Update the source schema constants in `bin/agent-container`: `CRED_SOURCES = ("env", "file", "keychain", "command", "onepassword", "bitwarden")` (**remove `encrypted`**), and extend the per-source required-field map in `validate_credential` — `command`→`("argv",)`, `onepassword`→`("vault", "item", "field")`, `bitwarden`→`("item", "field")` — keeping `env`/`file`/`keychain` unchanged.
...
- [X] T005 [P] [US1] Write failing tests in `bin/tests/test_agent_as_code.py` for the **`command`** source: `validate_credential` accepts a well-formed `{source: command, argv: [...]}` and **rejects** a non-list `argv`, an **empty** list, and a **non-string element** — each `die`ing naming the field (FR-015); unknown keys still rejected. `resolve_credential_value` runs **exactly the declared argv** (assert the argv passed to a mocked `_run_resolver`) and returns its stdout; a failing resolver `die`s **before any change** naming the credential and source (FR-004); the resolved value never appears in any log/`die` message (Constitution III); and a read-only **`plan`/`status` never invokes a resolver** (assert `_run_resolver` is not called for a spec carrying a `command` credential — a preview must not trigger a manager prompt or hardware-key touch, FR-002).
- [X] T006 [US1] Add the `command` branch to `resolve_credential_value` in `bin/agent-container` (dispatch `cred["argv"]` → `_run_resolver`), and add the **argv-list type validation** to `validate_credential` (non-empty list of strings, else `die` naming the field).
- [X] T007 [P] [US1] Acceptance in `bin/tests/test_acceptance.py`: a declarative project whose credential uses `{source: command, argv: ["printf", "<known>"]}` applies; the value is present in the running container (verifiable in-container) and **no plaintext appears in the project dir or the captured output** (SC-001); a resolver that exits non-zero fails the apply **before any change**, naming the credential (SC-002). Reuse the Feature 006 declarative acceptance harness.
- [X] T007a [P] [US1] **Delivery regression guard (FR-012)** — write tests in `bin/tests/test_agent_as_code.py` asserting `stage_declared_credentials` still routes a value resolved from a **new** source correctly through the unchanged Feature 003 channels: a **provider name** (`anthropic`/`openai`) → the **apikey file channel** (a config → `INJECT_APIKEY_DIR/<provider>`, never the env); an SSH **`target`** → the ssh channels; anything else → the per-deployment **0600 secrets env-file**. Also pin the **newline contract**: a resolver's trailing `"\n"` (as `op read` emits) is **stripped** for apikey/env delivery and **ensured** for SSH-key delivery, so a manager's stdout can never corrupt an API key. (research R6 promised this guard; it was missing from the task list — analyze C1.)
...
- [X] T009 [US2] Add the `onepassword` and `bitwarden` branches to `resolve_credential_value` in `bin/agent-container` — assemble the argv per research R3 and delegate to `_run_resolver`; the generic `command` source remains the extensible escape hatch for unnamed managers (FR-008).
...
- [X] T013 Run `scripts/quality-gate.sh` (ruff · ty · bandit · vulture · xenon · refurb · self-test · pytest · shell) and fix all findings; confirm **bandit is clean** (no `shell=True` anywhere in the new resolver paths) and that `resolve_credential_value` stays within xenon rank B (extract per-source helpers if the added branches push it over).
- [X] T014 [P] Update `README.md` and the `CLAUDE.md` Decisions bullet for Feature 006/008: the credential sources now include the no-shell `command` resolver + named `onepassword`/`bitwarden`, `encrypted` is **removed** (breaking, with a migration refusal), and the repo stores only a **locator** — within the CLAUDE.md 2000-token budget (prune before adding).
- [X] T015 Run quickstart.md Scenarios A–E (the named-manager scenario is opt-in — it needs a real, unlocked `op`/`bw` session; the rest are CI-safe) and record the results in quickstart.md.
- [X] T016 **Mark the breaking change (Principle VII)** — the commit that removes the `encrypted` source (T011) MUST carry a Conventional-Commits **`BREAKING CHANGE:`** footer naming the removed source and the migration (manager / OS keychain / external-untracked file), so python-semantic-release records it in `CHANGELOG.md` and an upgrading operator is warned. Verify after merge that the generated changelog entry contains it. (analyze C3.)
...
- US1: T005 ∥ T007 ∥ T007a; impl T006.
- US2: T008 ∥; impl T009.
- US3: T010 ∥ T012; impl T011.
- Polish: T014 `[P]`; T013/T015 sequential (gate, then record); T016 is a commit-message requirement on T011, verified after merge.
