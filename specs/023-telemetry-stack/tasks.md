...
- [x] T007 Implement `validate_stack_name(name, host)` in `bin/agent-container` refusing a name already held by an agent environment or control plane on that host, naming the kind that holds it (FR-009a)
- [x] T008 [P] Unit-test the name namespace in `bin/tests/test_pure_logic.py`: stack-vs-agent, agent-vs-stack, stack-vs-stack, and same name on DIFFERENT hosts (which must be allowed)
