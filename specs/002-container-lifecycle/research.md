...
**Rationale**: `--build` rebuilds the image on the host; `--force-recreate` guarantees the container is replaced even when compose's own change-detection would skip it, satisfying FR-008 (apply a changed image). `redeploy` is **deliberately non-idempotent** (resolved I1): the operator explicitly asked to rebuild, so it always rebuilds + recreates even with no change; it MAY warn "no change detected" but still performs the rebuild. The **idempotent** path is the inherited `up` (no `--force-recreate`) — compose recreates only if the image/config actually changed (FR-010). Volumes are pinned `{"name": vn}` (already the case from 001), so a recreate re-attaches them rather than making new ones.
