...
- [X] T001 [P] Add `bin/tests/test_credentialing.py` (new module) with hermetic fixtures for injected-material staging + the compose-model assertions (reuse the `wiz`/`make_registry` fixtures from `bin/tests/conftest.py`; no live runtime).
...
- [X] T002 [P] Write failing tests in `bin/tests/test_credentialing.py` / `bin/tests/test_command_construction.py`: an injected **secret** is staged locally (existence-validated), emitted as a compose `config` at an **ephemeral** `/run/agent-container/...` target, and its value never appears on argv nor inlined in the compose model; an injected **config** may target the canonical-config dir; canonical config (incl. MCP defs) is non-secret by definition (FR-007) and travels a separate channel from the ephemeral key-file secret channel (FR-009 — richer auto-classification of a secret-bearing config deferred to Feature 006). (FR-009/010/011/013/014, data-model invariant matrix.)
- [X] T003 Add the ephemeral inject-path constants — `INJECT_PUSH_KEY_PATH`, `INJECT_KNOWN_HOSTS_PATH`, `INJECT_APIKEY_DIR`, `INJECT_CONFIG_DIR` (all under `/run/agent-container/…`) — kept in sync with `entrypoint.sh`'s `INJECT_DIR`, in `bin/agent-container`.
- [X] T004 Implement the ephemeral staging helper (mirroring `stage_ssh_injection` but **ephemeral-class**: byte-copy the source **without reading its contents** for secret hygiene, `die` fast if a referenced source is absent (FR-016), stage under `<state>/<host>/<name>.*` so compose ships it over the context (FR-014), and mark it so the entrypoint does **not** persist it to a volume (FR-012)), in `bin/agent-container`. **All injected material for a deploy is staged + validated BEFORE any compose call** — the single fail-fast / no-partial-provisioning guard (FR-016/FR-017) that every user story inherits (US4 verifies it end-to-end, does not re-implement it).
- [X] T005 Extend `build_compose_model(...)` to accept and emit the new injected material — push key + known_hosts, model/API key file(s), canonical-config file(s) — as compose `configs` at their targets (ephemeral secrets → `/run/...`; canonical config → `INJECT_CONFIG_DIR`), with **no secret value inlined** and no identity-volume overlap; canonical config (incl. MCP defs) is non-secret (FR-007) and targets `INJECT_CONFIG_DIR`, while secrets travel the ephemeral key-file channel (FR-009 auto-classification of secret-bearing config deferred to Feature 006), in `bin/agent-container`.
...
- [X] T006 [P] [US1] Write failing tests in `bin/tests/test_credentialing.py`: `--push-key`/`--known-hosts` stage + thread into the compose model as ephemeral configs at `INJECT_PUSH_KEY_PATH`/`INJECT_KNOWN_HOSTS_PATH`; the push key is **distinct** from the host key (SC-008); neither key value is on argv nor inlined.
- [X] T007 [US1] Add `--push-key PATH` / `--known-hosts PATH` options to `up` and `redeploy` (+ env-file channel `SSH_PUSH_KEY_B64` / `PUSH_KNOWN_HOSTS` parity), validate the key (`validate_private_key`; reject encrypted), `die` fast if a referenced file is missing (FR-016), and thread through `compose_up_exec` → `build_compose_model`, in `bin/agent-container`.
- [X] T008 [US1] Wire `GIT_SSH_COMMAND` in `entrypoint.sh` from the injected push key + known_hosts (`ssh -i <push-key> -o IdentitiesOnly=yes -o UserKnownHostsFile=<known_hosts> -o StrictHostKeyChecking=accept-new`), reading the key **in place** from the ephemeral inject path and **NOT** copying it onto the `~/.ssh` volume (FR-012); keep it strictly separate from the inbound host-key install (SC-008); consume `SSH_PUSH_KEY_B64`/`PUSH_KNOWN_HOSTS`. The HTTPS+`GH_TOKEN` helper block is retained unchanged (layered).
- [X] T009 [P] [US1] Shell test in `bin/tests/test_entrypoint.sh`: with an injected push key, `GIT_SSH_COMMAND` is set to use it with `IdentitiesOnly`, the key is **NOT** written onto the `~/.ssh` volume, and the inbound host-key path is untouched.
- [X] T010 [P] [US1] Acceptance (opt-in/tokened) in `bin/tests/test_acceptance.py`: from inside a provisioned container, clone→commit→push over SSH completes with **zero** prompts (SC-001); after teardown no push-key copy remains on any volume (SC-004); the push key and host key are two distinct keys (SC-008). Real remote → outside the CI cost boundary.
...
- [X] T011 [P] [US2] Write failing tests in `bin/tests/test_credentialing.py`: a provider **key file** discovered at `./agent-container.<name>.<provider>.key` → `~/.config/agent-container/<name>.<provider>.key` (M2) is staged as an ephemeral config under `INJECT_APIKEY_DIR/<provider>`, targets an **ephemeral** path only (never a per-agent volume path — H1/FR-012), never on argv, never inlined; env/`.env` delivery remains supported (a `.env` key value never reaches argv).
- [X] T012 [US2] Implement provider-key **file discovery** at `agent-container.<name>.<provider>.key` → `~/.config/agent-container/<name>.<provider>.key` (`<provider>` ∈ `anthropic`/`openai`/…, lower-case; one file per provider — M2) + ephemeral staging + `build_compose_model` wiring to `INJECT_APIKEY_DIR/<provider>`; retain the existing env/`.env` delivery path, in `bin/agent-container`.
- [X] T013 [US2] Per-agent API-cred wiring in `entrypoint.sh`, **tool-injected credential ALWAYS ephemeral** (FR-012/SC-004, H1): Claude → `apiKeyHelper` in the fresh canonical `settings.json` that `cat`s the injected key at `INJECT_APIKEY_DIR/anthropic` (the `~/.claude` volume never receives the key); Codex → redirect `CODEX_HOME` to an ephemeral `/run/agent-container/codex-home` + `codex login --with-api-key` on **stdin** (or `OPENAI_API_KEY` in the in-container env if honored) — the `-codex` volume is **never** written; pi → redirect `PI_CODING_AGENT_DIR` to an ephemeral `/run` dir (or provider key in env) — the `-pi` volume is **never** written. A non-interactive `login` to a persistent volume is NOT permitted; on-volume `auth.json` is **operator-interactive only**. Absent injected key → retain env/`.env` + interactive-login (NOTE, not `die`).
- [X] T014 [P] [US2] Acceptance (opt-in/tokened) in `bin/tests/test_acceptance.py`: a provisioned agent performs a backend-requiring operation (SC-002); assert the key is absent from argv, the compose file, image layers, and persistent volumes (SC-003/SC-004). Never in CI (no cost/secret).
...
- [X] T015 [P] [US3] Write failing tests in `bin/tests/test_credentialing.py`: canonical-config files (per the per-agent manifest) discovered from the `agent-container.<name>.config/` convention are emitted as configs at `INJECT_CONFIG_DIR`; runtime-state paths are NOT delivered.
- [X] T016 [US3] Implement the per-agent **canonical manifest** (the operator-owned paths per data-model — e.g. `~/.claude/settings.json` + `CLAUDE.md` + MCP defs; `~/.codex/config.toml`; `~/.pi` config) + `agent-container.<name>.config/` → `~/.config/agent-container/<name>.config/` discovery + staging to `INJECT_CONFIG_DIR`, in `bin/agent-container`.
- [X] T017 [US3] Add the canonical-copy step to `entrypoint.sh`: on each boot, copy the manifest's files from `INJECT_CONFIG_DIR` onto their per-agent volume paths (overwrite the canonical files), leaving all other (runtime-state) files under the home untouched; idempotent so `redeploy` re-applies edits (FR-007/FR-008).
- [X] T018 [P] [US3] Acceptance in `bin/tests/test_acceptance.py`: edit a canonical file → `redeploy` → the change is reflected in the container; write runtime state → dispose+recreate → the state survives from the per-agent volume (SC-005).
...
- [X] T019 [P] [US4] Write failing tests in `bin/tests/test_credentialing.py`: a deploy referencing a missing `--push-key`/`--known-hosts`/API/canonical file `die`s **before** any compose call (FR-016/SC-007); staging is all-local-before-`compose up` so a later failure leaves nothing running (FR-017).
- [X] T020 [US4] **Verify** the deploy path (`do_up`/`compose_up_exec`) honors the foundational all-staging-before-`compose up` guard (T004) end-to-end across every material type — a missing/invalid item of ANY kind aborts before the container is created, no partially-credentialed agent (FR-016/FR-017) — and that a narrowly-scoped per-repo deploy key is simply a narrower `--push-key` (FR-004/FR-013), in `bin/agent-container`.
- [X] T021 [P] [US4] Acceptance (opt-in/tokened) in `bin/tests/test_acceptance.py`: rotate a secret (edit local + `redeploy`) → new value in effect, no baked/persisted copy of the old (SC-006); a per-repo deploy key grants only the intended scope (FR-004).
...
- [X] T022 [P] Update `docs/credentials.md` (FR-018): add the **SSH push** section as the documented default (deploy key + known_hosts, ephemeral, `GIT_SSH_COMMAND`), keep **HTTPS+`GH_TOKEN`** as the documented alternative, and record the ephemeral-not-persisted discipline (FR-012) + the two-key distinction (SC-008). Supersede the "SSH push rejected for MVP" note.
- [X] T023 [P] Update `.env.example` (FR-018): the `SSH_PUSH_KEY_B64` / `PUSH_KNOWN_HOSTS` channel and the file-first API/config notes.
- [X] T024 [P] Update `README.md` + `CLAUDE.md` (FR-018): the credential model (push key, file-first API, canonical/runtime split) — within `CLAUDE.md`'s 2000-token budget, prune before adding.
- [X] T025 Run `scripts/quality-gate.sh` (ruff · ty · bandit · vulture · xenon · refurb · self-test · pytest · shell suites) and fix all findings.
- [X] T026 Run quickstart.md Scenarios A–G (local + the opt-in/tokened ones) and record the results in quickstart.md.
