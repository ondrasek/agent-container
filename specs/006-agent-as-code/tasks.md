...
- [X] T018 [US4] Add host-binding resolution to the apply/destroy paths: `referenced` (existing/known host — externally owned) vs `provisioned` (drive the 001 `host add`/provisioner, spec-owned); `destroy --deprovision` removes a spec-created host only with explicit intent (FR-017); in `bin/agent-container`.
- [X] T019 [P] [US4] Acceptance in `bin/tests/test_acceptance.py`: a **referenced**-host spec deploys and `destroy` leaves the host intact. The **provisioned**-host end-to-end (real Hetzner) is **opt-in/tokened** (billable — never in CI; behind `HCLOUD_TOKEN` + the existing acceptance opt-in guards).
...
- [X] T024 Run quickstart.md Scenarios A–H (local; the provisioned-host + real-agent ones are opt-in/tokened) and record the results in quickstart.md.
