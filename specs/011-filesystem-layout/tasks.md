...
- [ ] T005 [P] [US1] In `bin/tests/test_pure_logic.py`, assert the four-step env chain in order: `.agent-container/<name>.env` → `.agent-container/.env` → `~/.config/agent-container/<name>.env` → `~/.config/agent-container/.env`, and that the bare `./.env` is **not** in it (contract C2, FR-001b).
- [ ] T006 [P] [US1] In `bin/tests/test_credentialing.py`, assert credential, canonical-config and sidecar resolution all read from `.agent-container/<name>.*` with the user-level fallback, and that **the same filename means the same thing at both levels** (FR-001a).
...
- [ ] T008 [P] [US1] In `bin/tests/test_credentialing.py`, assert the refusal matrix (contract C3): each superseded `agent-container.<name>.*` name refuses and names its destination; **all** offenders are listed in one message; and a `./.env` refuses **only** when no agent-container env file resolves, staying silent otherwise (FR-001c). The silent case is as load-bearing as the loud one — refusing on a Compose-owned `.env` is its own bug.
...
- [ ] T010 [US1] Rewrite `env_file_candidates` in `bin/agent-container` (line ~656) to the four-step chain from T005, dropping `cwd / ".env"`. Update its doctest — it currently asserts `/w/.env` as the first candidate.
- [ ] T011 [US1] Repoint `discover_apikey_files` (line ~681) at `project_config_dir(cwd)` with the `<name>.<provider>.key` form, dropping the `agent-container.<name>.` prefix.
- [ ] T012 [US1] Repoint `canonical_config_dir` (line ~731) and `discover_canonical_config` (line ~744) at `.agent-container/<name>.config/`.
- [ ] T013 [US1] Repoint `resolve_sidecar_override` (line ~2010) at `.agent-container/<name>.services.yaml`. Its doctest names both old paths and must be updated.
