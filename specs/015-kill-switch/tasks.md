...
- [X] T001 `KILL_HOST_TIMEOUT = 30.0` in `bin/agent-container`, with a comment stating its **floor**:
      ...
      inherits the bug
- [X] T002 [P] `KILL_OUTCOMES = ("stopped", "already-stopped", "failed", "undetermined")` — closed at
...
- [X] T003 `kill_candidates(scope)` in `bin/agent-container` — the inventory's **`active`** entries
...
- [X] T004 The scope resolves from **stored fields alone, contacting no host** (FR-011, clarified).
      Scoping that needed a daemon would depend on the very reachability this feature cannot assume
- [X] T005 `kill_read_inventory()` — distinguishes **unreadable** (refuse, naming the store) from
...
- [X] T006 [P] Unit tests for T005 both ways: an unreadable store refuses **and does not enumerate
      live**; an absent one exits 0 with wording that does not imply nothing exists
- [X] T007 `project_containers(host_rec, project, include_stopped)` — enumerate a deployment's
      ...
      sidecars
- [X] T008 [P] Unit test that T007 builds a label filter and **never** references
      `compose_file_path` — the regression this feature exists to avoid, asserted over the call rather
      than trusted
- [X] T009a `kill_snapshot(host_rec, form)` — the **pre-snapshot** of what is running on a host,
...
- [X] T009 `kill_verify(host_rec, form)` — ONE re-query per host after its work (FR-014, C4, C5).
...
- [X] T010 [P] Unit tests for T009: a stopped-but-existing container verifies as stopped under `stop`
      and would NOT verify under `destroy` — the test that catches the conflation
- [X] T011 `classify(entry, before, after, form)` returning exactly one `KILL_OUTCOMES` value
      (FR-004, data-model §3, C3). Every entry lands in exactly one; zero unclassified
- [X] T012 [P] Unit test that `undetermined` is produced for **both** its causes — an unreachable host
...
- [X] T013 [US1] `do_kill(scope, form, preview)` in `bin/agent-container` — enumerate, act per host,
      verify, classify, report (FR-001, C1)
- [X] T014 [US1] **THE GATE: the unreachable host** (C3, SC-002, S2). A host that does not answer
...
- [X] T015 [US1] [P] Unit test for T014 asserting the ABSENCE of `stopped` for the unreachable host
...
- [X] T016 [US1] Per-host parallelism with a per-host timeout (C5); **environments sequential within a
...
- [X] T017 [US1] One host's failure does not abort the others (FR-003, C2) — each host's task is
      independent, and a timeout or exception yields `undetermined` for its environments only
- [X] T018 [US1] [P] Unit test T017 by making one host raise: the others still complete. A run that
      aborts on first failure leaves an operator worse off than doing it by hand
- [X] T019 [US1] `stopped` is written **only** after observation (FR-014, SC-002b). A command exiting
      zero is not evidence and must not reach the classifier as one
- [X] T020 [US1] Exit status follows the worst outcome: anything not `stopped`/`already-stopped` means
...
- [X] T021 [US1] [P] Unit test that a single `undetermined` among many `stopped` still fails the run
- [X] T022 [US1] A contended `deployment_lock` yields `undetermined`, never a `die` and never a silent
      skip (research R5). Dying violates FR-003; skipping reports success for something never touched
- [X] T023 [US1] [P] Acceptance S2 in `bin/tests/test_acceptance.py` — real containers, one host made
      unreachable: reachable stop, unreachable `undetermined`, non-zero exit
- [X] T024 [US1] [P] Acceptance S5 — every environment reported `stopped` is **absent from the running
      listing**, and still present in `ps -a` (which is correct for a stop)
- [X] T025 [US1] [P] Acceptance S1 + S3 (SC-001) — everything stops; one failure does not abort the rest
...
- [X] T026 [US2] The **stopping** form: halt the project's containers, volumes untouched (FR-006,
      SC-005)
- [X] T027 [US2] The **destroying** form with **purge reach** — containers and their volumes, and
...
- [X] T028 [US2] [P] Test that the destroying form leaves images intact — the assertion that keeps
      `destroy` from drifting into `wipe`
- [X] T029 [US2] `destroy` requires explicit confirmation; **`stop` requires none** (FR-007, C7). The
      asymmetry is deliberate: stopping is recoverable and a prompt is friction on the action whose
      value is speed
- [X] T030 [US2] [P] Acceptance S7 (SC-005, SC-006) — volumes survive a stop; `destroy` without `-y`
      performs **zero** destructive operations
- [X] T031 [US2] `--preview` prints exactly what would be affected and **changes nothing** (FR-008,
      ...
      *undetermined*
- [X] T031a [US2] Preview **never hangs and never fails the invocation**: an unreachable host during a
...
- [X] T032 [US2] [P] Acceptance S8 — capture container and volume state before and after a preview
      and assert they are identical. Contacting a host is a read; SC-007 is unaffected by T031
- [X] T033 [US2] Write back per data-model §4: a **stop** appends to the entry's `notes`, **retaining
...
- [X] T033a [US2] **An `undetermined` or `failed` destroy writes NOTHING** — the entry stays `active`
...
- [X] T033ab [US2] [P] Unit test the `notes` cap: six runs leave five notes, newest kept, and the
...
- [X] T033b [US2] [P] Unit test T033a: a destroy against an unreachable host leaves the entry
...
- [X] T034 [US2] [P] Unit test that a stop leaves `outcome` untouched while recording what happened,
      and that a destroy marks `removed` — the pair that proves §4 rather than either half alone
- [X] T035 [US2] Document **which form suits which emergency** in `docs/orchestration.md` (FR-006a,
...
- [X] T036 [US3] `--host` and `--name`, both repeatable, applied to T003's candidates (FR-011)
- [X] T037 [US3] The report **states what the scope excluded** (FR-011, C12) — a kill switch that
      silently narrowed itself is the same false guarantee as one that fell back to live enumeration
- [X] T038 [US3] A scope matching nothing **says so** rather than silently doing nothing (C12). When
...
- [X] T039 [US3] [P] Acceptance S11 — one host scoped, others untouched, exclusions named

---

## Phase 6: The honest edges

- [X] T040 **The compose project label is the ownership boundary** (FR-009, C10, clarified): every
...
- [X] T041 [P] Acceptance S9 — create `agent-container-impostor` with a plain `docker run`, kill, and
...
- [X] T047 `--json` carries the full result including per-environment outcomes (FR-015, C15), through
      the existing envelope
- [X] T048 [P] Completions for the new command in both shells, plus the assertion in
