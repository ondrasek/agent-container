...
```sh
agent-container panic --json | jq '.data.results[] | {name, host, outcome}'
echo "exit=$?"
```
...
```sh
time agent-container panic --host-timeout 10
```
...
```sh
agent-container panic --json | jq '.data.results[] | select(.outcome=="stopped")'
...
```sh
agent-container panic                       # stop
docker volume ls | grep agent-container-   # volumes intact

agent-container panic --destroy             # no -y
echo "exit=$?"
...
agent-container panic --preview
docker ps -a --format '{{.Names}} {{.Status}}' | sort > /tmp/after
diff /tmp/before /tmp/after && echo "unchanged"
```

**Expect**: identical, and the preview listed exactly what a real run would target. The preview
**does** contact hosts (so it shows what is actually running), which is a read — SC-007 is about
state change, and reading changes none.
...
```sh
agent-container panic && agent-container panic; echo "exit=$?"
```
...
```sh
agent-container panic --host vps1
agent-container panic --host nosuchhost
```
...
agent-container panic; echo "exit=$?"          # REFUSES, naming the store
chmod 755 "${XDG_DATA_HOME:-$HOME/.local/share}/agent-container/inventory"

mv .../inventory .../inventory.bak
agent-container panic; echo "exit=$?"          # succeeds: "nothing recorded"
```
