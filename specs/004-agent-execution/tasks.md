...
- [X] T001 [P] Add `bin/tests/test_execution.py` (new module) with hermetic fixtures for the compose-model / flag assertions (reuse the `wiz`/`make_registry` fixtures from `bin/tests/conftest.py`; no live runtime).
...
- [X] T006 [P] [US1] Write failing tests in `bin/tests/test_execution.py`: `up`'s `--mode`/`--agent`/`--task` thread into the compose env + the task inject; interactive sets `restart: unless-stopped`.
- [X] T007 [US1] Add `--mode`/`--agent`/`--task` options to `up` (and `redeploy`), resolve `@file` for `--task`, and thread through `do_up`/`do_redeploy` → `compose_up_exec` → `build_compose_model`, in `bin/agent-container`.
- [X] T008 [US1] In `entrypoint.sh`, branch on `AGENT_CONTAINER_MODE` (default interactive = the existing sshd+tmux flow) and, in interactive mode, **launch `AGENT_CONTAINER_AGENT` in a dedicated tmux window** seeded with the injected task if present (per-agent invocation map: claude/codex/pi); PID 1 stays alive as today.
- [X] T009 [P] [US1] Shell test in `bin/tests/test_entrypoint.sh`: with `AGENT_CONTAINER_MODE=interactive` + an agent + a task file, the agent is launched in a tmux window with the task; bare-shell windows still exist; no agent launched when `--agent`/task absent (backward-compatible default).
- [X] T010 [P] [US1] Acceptance in `bin/tests/test_acceptance.py`: `up --mode interactive` → a tmux window running the agent process exists; a `--task` deployment shows the agent started on the task. (The agent actually *responding* — SC-001 — is the opt-in/tokened extension.)
...
- [X] T011 [P] [US2] Write failing tests in `bin/tests/test_execution.py` / `test_command_construction.py`: the attach path issues an explicit `tmux has-session -t main` probe and maps its result to (attach / fresh session / clear "nothing running") — never a silent empty attach.
- [X] T012 [US2] Implement the dead-session probe in the attach path (`cli_attach`/`ssh_argv`/`wizard_handover`): before/at attach, check `tmux has-session -t main`; on a live session attach as today; on none, present a freshly (re)started session OR `die`/report "nothing running" clearly (FR-008); reattach stays machine-independent (FR-007), in `bin/agent-container`.
...
- [X] T019 [US4] Add `--workspace`/`--workspace-dir`/`--repo` to `up`/`redeploy`, wire workspace resolution (T004) + the clone-credential fail-fast pre-check (SSH-URL repo requires an injected push key, FR-014) before `compose up`, in `bin/agent-container`.
- [X] T020 [US4] Add **clone-on-start** to `entrypoint.sh`: for persistent/ephemeral with `AGENT_CONTAINER_CLONE_URL` set and `/workspace` empty, `git clone` choosing the credential by URL scheme (`git@…` → the injected push key via `core.sshCommand`; `https://…` → `GH_TOKEN`); `die` on an SSH URL with no key (FR-014); **idempotent** (skip if a working copy exists); a bind workspace is never cloned; runs BEFORE the agent launch (T008/T015).
- [X] T021 [US4] Surface the ephemeral **durability warning** (FR-015) at deploy (a clear NOTE that uncommitted ephemeral work is lost on teardown), in `bin/agent-container` (and/or entrypoint log).
- [X] T022 [P] [US4] Acceptance in `bin/tests/test_acceptance.py`: persistent retains its working copy across recreate (SC-006); bind edits appear on the local FS and a remote bind is refused (SC-007); ephemeral is gone after teardown (SC-006); clone-on-start populates `/workspace` (HTTPS) and fails fast on an SSH URL with no key (SC-008).
...
- [X] T023 [P] Update `README.md`: execution modes (interactive/headless, `--mode`/`--agent`/`--task`/`--foreground`), detach/reattach + dead-session behavior, workspace modes + `--workspace`/`--repo`, clone-on-start (FR-018).
- [X] T024 [P] Update `CLAUDE.md` Decisions (execution modes, workspace-mode + conditional workspace volume, clone-on-start) within the 2000-token budget — prune before adding (FR-018).
- [X] T025 [P] Update the relevant `docs/` (e.g. a docs/execution.md or the existing docs) with the mode/session/workspace semantics + the ephemeral-durability caveat (FR-015/FR-018).
- [X] T026 Run `scripts/quality-gate.sh` (ruff · ty · bandit · vulture · xenon · refurb · self-test · pytest · shell suites) and fix all findings.
- [X] T027 Run quickstart.md Scenarios A–H (local + the opt-in/tokened agent-responds ones) and record the results in quickstart.md.
