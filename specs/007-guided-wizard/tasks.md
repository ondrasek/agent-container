...
- [ ] T001 [P] Add `bin/tests/test_guided_wizard.py` (new module) with hermetic fixtures: reuse the `wiz` fixture from `bin/tests/conftest.py`; a factory that builds an `EnvSnapshot` from plain args (no live runtime, no TTY) so every engine rule is asserted against an injected snapshot.
...
- [ ] T003 Add the dataclasses in `bin/agent-container`: `StageStatus` (enum), `SetupStage`, `ActiveTarget`, `EnvSnapshot`, `RecommendedAction`, `ActionOutcome` (fields per data-model.md), with `RecommendedAction.equivalent_cmd` secret-free by construction.
- [ ] T004 Add `assess_stages(snapshot) -> list[SetupStage]` (pure, no I/O) — the ordered tri-state assessment walking the fixed prerequisite chain (FR-016); `credentials` is soft (FR-018); in `bin/agent-container`.
- [ ] T005 Add `build_snapshot(target) -> EnvSnapshot` — the thin assembler mapping each stage to its **existing** probe (`detect_runtime`/`probe_host_runtime`, `image_exists`, `resolve_env_file`, `host_ps_rows`, `probe_session`), **bounded to the active target's host** (FR-017), plus the host container inventory and the orphan-volume scan; in `bin/agent-container`.
- [ ] T006 Add `resolve_active_target(selected_host, selected_name) -> ActiveTarget` — host via `resolve_deploy_host` (implicit-local default; `ambiguous_host` when >1 registered and none selected), container name reusing the sole existing container else a default (FR-017/019, reusing `container_name`); in `bin/agent-container`.
...
- [ ] T008 [US1] Add `recommend_next_step(snapshot) -> RecommendedAction` forward-progress logic (first unsatisfied **hard** stage → its action; all hard satisfied + not running → `start`; running → `attach`), each with a `reason` and `equivalent_cmd`; keep xenon rank B via per-case helpers; in `bin/agent-container`.
- [ ] T009 [US1] Rewrite `wizard_loop` as **gather → render → act → re-evaluate**: `build_snapshot(resolve_active_target(...))` → show the state summary + the single marked recommendation → perform via existing handlers (`wizard_start`/`wizard_attach`, the build path) → re-run on a fresh snapshot (FR-005); keep the no-TTY guard (FR-013) and add the container-naming prompt (FR-019); in `bin/agent-container`.
- [ ] T010 [P] [US1] Acceptance in `bin/tests/test_acceptance.py`: on a real local runtime with no container, driving the wizard's recommended steps (scripted input) reaches a running/attachable container (SC-001); and the no-TTY invocation declines cleanly pointing to the subcommands (FR-013).
...
- [ ] T012 [US2] Extend `recommend_next_step` for the healthy/running case (day-to-day action) and add the **which-one** target-selection hook consumed by the shell (FR-014, reusing `host_ps_rows`); in `bin/agent-container`.
...
- [ ] T014 [US3] Add problem detection to `build_snapshot` (the R4 signal→problem mapping) and the **corrective-precedence** branch in `recommend_next_step` (a detected problem outranks forward progress); keep complexity in budget; in `bin/agent-container`.
...
- [ ] T015 [P] [US4] Write failing tests in `bin/tests/test_guided_wizard.py`: `valid_actions(snapshot)` lists **every** currently-valid action (escape hatch, SC-007); the recommendation is flagged distinct from the alternatives (FR-002); every action's `equivalent_cmd` is present and **secret-free** for a snapshot carrying a resolved credential (SC-006, Constitution III); destructive actions carry the confirm flag (FR-011).
- [ ] T016 [US4] Add `valid_actions(snapshot) -> list[RecommendedAction]` and wire the shell to: render the compact state summary + the marked recommendation (FR-009/002), offer the escape-hatch actions (FR-008), show the equivalent command for the chosen action (FR-010), confirm destructive ones (FR-011), and on a failed action **report + re-evaluate** rather than advance (FR-012); in `bin/agent-container`.
...
- [ ] T017 Run `scripts/quality-gate.sh` (ruff · ty · bandit · vulture · xenon · refurb · self-test · pytest · shell) and fix all findings; confirm `recommend_next_step`/`build_snapshot` stay within xenon rank B (extract helpers if needed).
- [ ] T018 [P] Update `README.md` and the `CLAUDE.md` Decisions bullet: the guided wizard (state-aware next-step guidance replacing the flat menu; the pure-engine/thin-shell split; single active target with bounded probing; soft credentials; the equivalent-command teaching aid) — within the CLAUDE.md 2000-token budget (prune before adding).
- [ ] T019 Run quickstart.md Scenarios A–G (local) and record the results in quickstart.md.
