...
- [~] T138 [US5] Acceptance: `git push` over declared SSH **succeeds** (quickstart S18). If this fails the feature is unshippable, whatever else passes  — **PARTIAL, and the gap is named rather than ticked over.** The test exists and passes, but it asserts the TRANSPORT arm only: the verification pass had no push key available, so an actual `git push` over declared SSH has never run. Read under R24 — the rule pins one rotation address, and the test probes seconds after `up`, so it cannot observe the failure this task exists to prevent.
