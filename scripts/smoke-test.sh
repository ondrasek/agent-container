...
#   - uv on PATH (agent-container is a PEP 723 uv script) and bin/agent-container present
#   - ./.env present with GH_TOKEN, GIT_USER_NAME, GIT_USER_EMAIL non-empty
#   - AGENT_CONTAINER_SMOKE_REPO env var: "owner/name" of a GitHub repo your GH_TOKEN
...
# agent-container is a single PEP 723 script; invoke it via uv (array preserves argv).
readonly AGENT_CONTAINER_BIN="${REPO_ROOT}/bin/agent-container"
readonly AGENT_CONTAINER=(uv run --no-project --script "${AGENT_CONTAINER_BIN}")
readonly INSTANCE="smoketest"
readonly STATE_DIR="${XDG_STATE_HOME:-$HOME/.local/state}/agent-container"
...
cleanup() {
    local rc=$?
    phase "cleanup"
    if [[ -f "${AGENT_CONTAINER_BIN}" ]]; then
        "${AGENT_CONTAINER[@]}" down "$INSTANCE" --purge >/dev/null 2>&1 || true
        info "container agent-container-${INSTANCE} removed (purged volume)"
    fi
...
VERIFY_DIR="$(mktemp -d -t agent-container-smoke.XXXXXX)"
...
command -v uv >/dev/null 2>&1 || fail "uv not on PATH (agent-container is a uv script)"
[[ -f "${AGENT_CONTAINER_BIN}" ]] || fail "bin/agent-container missing"
info "uv + bin/agent-container ready"
...
: "${AGENT_CONTAINER_SMOKE_REPO:?AGENT_CONTAINER_SMOKE_REPO must be set to owner/name of a GitHub repo your GH_TOKEN can push to}"
info "target repo: ${AGENT_CONTAINER_SMOKE_REPO}"

# --- Phase 2: build ---------------------------------------------------------
phase "build image"
"${AGENT_CONTAINER[@]}" build >/dev/null
info "image built"

# --- Phase 3: up ------------------------------------------------------------
phase "launch container"
"${AGENT_CONTAINER[@]}" up "$INSTANCE"
...
for i in {1..30}; do
    ...
    if [[ $i -eq 30 ]]; then
        ...
        "${AGENT_CONTAINER[@]}" logs "$INSTANCE" >&2 || true
...
RUNTIME="${AGENT_CONTAINER_RUNTIME:-}"
if [[ -z "$RUNTIME" ]]; then
...
CLONE_URL="https://github.com/${AGENT_CONTAINER_SMOKE_REPO}.git"
...
"$RUNTIME" exec -i "agent-container-${INSTANCE}" bash -s <<INNER
set -euo pipefail
cd /workspace
rm -rf smoke-target
git clone "${CLONE_URL}" smoke-target >/dev/null 2>&1 \\
    || { echo "[in-container] clone failed — check AGENT_CONTAINER_SMOKE_REPO and GH_TOKEN scope" >&2; exit 11; }
cd smoke-target
...
GIT_ASKPASS=/bin/true \
    ...
    || fail "could not re-clone ${AGENT_CONTAINER_SMOKE_REPO} from host"
