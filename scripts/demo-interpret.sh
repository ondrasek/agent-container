...
# NO EGRESS BLOCK, and that is a finding rather than a shortcut.
#
# The first version declared one, following the remedy `telemetry stack up`
# prints verbatim:
#
#     egress:
#       allow:
...
#           port: 4424
#
# Every collector connection was still REFUSED at the boundary for the whole
# run -- `agent-container egress <name>` shows them, every 30s -- so all three
# records came back "the telemetry endpoint did not answer; this record is
# local only". Note also that `enforcement: advisory` does NOT mean "record but
# allow": the mode decides whether to deploy when enforcement is impossible,
# and an enforceable boundary enforces either way.
#
# Until that is fixed, a demo that declares egress demonstrates the egress bug
# instead of the interpreter. The runs reach ollama.com and github.com
# unfiltered, which is the pre-012 posture and fine for a throwaway root.
write_spec() {
...
leave the commit local. This is deliberate."
    ...
    # The interpreter is DECLARED alongside the agents and comes up with them:
    # `container.role: interpreter` plus its own `interpreter:` block. It used
    # to need an imperative `up --role interpreter`, because the spec had no
    # role field at all.
    cmd_runbook
}
